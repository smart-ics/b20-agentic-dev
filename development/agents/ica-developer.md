...
- IN-PROGRESS -> resume: re-dispatch an ica-implementer to complete the slice.
...
2. Dispatch the subagent required by that slice's current state: a FRESH
   ica-implementer for implement/resume/remediate, or a FRESH ica-reviewer for
   review/re-review. Pass the slice ID, objective, dependencies, ARCHITECTURE,
   the plan path, and any prior findings. The ica-implementer and ica-reviewer for the
   same slice must always be separate agent executions.
3. Evaluate a review decision:
- GO: read the plan-level Status from IMPLEMENTATION-PLAN. The ica-reviewer
      ...
      - If Status is not COMPLETED, verify the COMPLETED condition yourself. If
        it is satisfied but the ica-reviewer did not set COMPLETED, flag the
        mismatch and escalate to the ica-architect instead of finishing. If it is
    ...
    - NO-GO: read the remediation count from the slice's `<CODE>-REVIEW.md`
      ...
      - If the count is less than 2, dispatch a fresh ica-implementer for
        remediation with the findings attached, then a fresh ica-reviewer for
        re-review.
      - If the count is 2 or greater, STOP the slice: park it, preserve all
        findings and remediation history, escalate to the ica-architect, and stop
        the run with a final report. No third remediation.
4. Stop conditions:
    - A slice returns NO-GO after its 2nd remediation -> stop that slice and
      escalate (as above).
    - The ica-implementer marks a slice BLOCKED (architecture cannot be satisfied)
      -> stop that slice, escalate to the ica-architect with the ica-implementer's
      rationale, and continue only with slices that do not depend on it.
    - The ica-implementer or ica-reviewer reports a structural plan defect -> stop and
      escalate to the ica-architect for a replacement plan. Never patch the
...
- Never let the ica-implementer and ica-reviewer share a single execution for the
  same slice.
