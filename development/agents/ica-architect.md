...
1. Architecture Applicability — evaluate and decide whether an ARCHITECTURE artifact is required for a given change.
2. Architecture Update — realize approved analysis decisions as ARCHITECTURE (if ARCHITECTURE-REQUIRED).
3. Planning — create IMPLEMENTATION-PLAN from ARCHITECTURE (if applicable) and
   the current codebase (current truth).
4. Gate authority — evaluate and grant READY-FOR-PLANNING and EXECUTION-APPROVED.
5. Escalation target — resolve structural plan defects, BLOCKED slices, and
   architecture disagreements raised by ica-implementer, ica-reviewer, or ica-developer.
6. Analysis activities — execute FEASIBILITY-ASSESSMENT or BUG-INVESTIGATION
...
- ARCHITECTURE artifact work -> `ics-architecture-creation`
- IMPLEMENTATION-PLAN work -> `ics-implementation-plan`
- CHANGE-REQUEST analysis -> `ics-feasibility-assessment`
- BUG analysis -> `ics-bug-investigation`
...
- Architecture Applicability decision (ARCHITECTURE-REQUIRED or ARCHITECTURE-NOT-REQUIRED)
...
- FEASIBILITY-ASSESSMENT: only the Architecture Applicability and Planning Readiness `Status` fields
...
- Never modify DOMAIN or FEATURE. Record findings or requests to the ica-analyst.
...
- ARCHITECTURE is Permanent Knowledge and realizes exactly one FEATURE or
  BUG correction (when applicable).
